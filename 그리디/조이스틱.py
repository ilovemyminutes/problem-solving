...
def get_char_distance(char1: str, char2: str) -> int:
...
def get_nearest_change_point(origin: int, meta: list, visited: list) -> (int, int):
    '''
    ...
    if all(visited[:origin] + visited[origin:]):
        ...
        while not all(visited):
            if meta[modular(origin-offset, len(meta))] == 1 and \
                visited[modular(origin-offset, len(meta))] == 0:
                left = modular(origin-offset, len(meta))
                break
            offset += 1
        # right
        ...
        while not all(visited):
            if meta[modular(origin+offset, len(meta))] == 1 and \
                visited[modular(origin+offset, len(meta))] == 0:
                right = modular(origin+offset, len(meta))
                break
            offset += 1
...
def modular(number:int, mod:int):
    '''모듈러 함수. get_nearest_change_point() 함수의 인덱싱 과정에서 IndexError 방지'''
    return number % mod
...
def solution(name):
    ...
    while not all(visited):
        ...
        if meta[idx] == 1: # 변화가 필요한 알파벳일 경우
            answer += get_char_distance('A', name[idx])
        post = get_nearest_change_point(idx, meta, visited)
        if post is None:
            break
        else:
            idx, distance = post
