                ...
                for order in sorted(
                    [
                        ...
                        for i, j in enumerate(
                            list(map(lambda x: x + x[-1] * (max_len - len(x)), temp))
                        )
                    ],
                    key=lambda x: x[-1],
                    reverse=True,
...
def solution(numbers):
    ...
    max_len = max(map(lambda x: len(str(x[-1])), idx_tup))
    print(max_len)
...
def get_order(idx_tup, max_len):
    padding = tuple(
        map(
            lambda x: int(str(x[-1]) + str(x[-1])[-1] * (max_len - len(str(x[-1])))),
            idx_tup,
        )
    )
    padding_indexed = sorted(
        ((idx, num) for idx, num in enumerate(padding)),
        key=lambda x: x[-1],
        reverse=True,
    )
