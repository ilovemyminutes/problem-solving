...
- 정체
  - 최단경로 탐색 알고리즘
  - 특정한 정점에서 다른 모든 정점으로 가는 모든 최단 경로를 알 수 있음
  - 음의 가중치를 지닌 간선은 포함할 수 없음
  - 그리디 알고리즘으로 분류됨: '*매 선택마다 최소 비용의 정점을 선택: 탐-욕*'
  - 다익스트라가 제안한 최단 경로 알고리즘
  - 노드 수를 V라고 할 떄, 다익스트라 알고리즘의 시간복잡도는 O(V^2)이기 때문에, 전체 노드 수가 5000개 이하일 경우 다익스트라 알고리즘을 통해 최단경로 문제를 풀 수 있음
    - 우선순위 큐를 활용하면 엣지 수를 E, 노드 수를 V라고 하면 O(ElogV)의 시간복잡도
- 로직
  - 첫 정점을 기준으로 연결된 정점들을 추가해나가며 최단 거리를 갱신: 현재까지 알고 있던 최단 경로를 계속해서 갱신
  - 정점을 잇기 전까지는 시작점을 제외한 정점들은 모두 무한대 값을 가짐
- 작동 과정
  1. 출발 노드 설정
  2. 최단 거리 테이블 초기화
     - 처음에는 기본적으로 자기 자신으로 가는 거리를 0으로, 그 외 노드까지의 거리를 무한으로 설정
     - 초기화 이후에는 다음의 3, 4 과정을 방문
  3. 방문하지 않은 노드 중에서 최단 거리가 가장 짧은(=가장 비용이 적은) 노드 선택
     - 그리디 알고리즘으로써의 특징이 여기에 있음
     - 더 짧은 거리의 경로를 찾을 떄마다 경로를 반복적으로 찾는 것과 최단 경로를 찾는 것은 동치
  4. '**<u>해당 노드를 거쳐</u>**' 특정 노드로 가는 경우를 고려해 최소 비용을 갱신
  5. 3, 4 과정을 반복
     - 일반적인 다익스트라 알고리즘만을 활용하면 *'최단 거리'*를 구할 수 있으며, ***'최단 경로'***를 구하기 위해서는 추가적인 구현 필요
     - 최단 경로 출력 문제는 일반적인 코테에서는 잘 출제되지 않음

- Reference
  - [다익스트라 알고리즘](https://hsp1116.tistory.com/42)
  - [23. 다익스트라 알고리즘](https://m.blog.naver.com/ndb796/221234424646)
...
* 스택은 가장 늦게 온 것이 먼저 나가고, 큐는 가장 먼저 온 것이 먼저 나가는 것처럼, 우선순위 큐는 ***가장 우선순위가 높은 것***이 먼저 나감
